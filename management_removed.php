...
for($k = 0; $k < $totalCount; $k ++) {
        ...
        if($row = mysqli_fetch_array($result)) {
                echo '<td><table><form action="recover_item.php" method="post"><tr><td style="width:230px">'.$row[name].
					 ...
					 '</td></tr><tr><td><img src="Picture/'.$row[filename].'" width="216" height="162" class="img-rounded"></td></tr><tr><td><center><input type="submit" class="btn btn-default" value="恢復上架"><input type="hidden" name="id" value="'.$row['id'].'"/></center></td></tr></form></table></td>';
        }
        else {
...
$sql2 = "select * from item_wanted where owner='$username' and status=2 order by id desc";
