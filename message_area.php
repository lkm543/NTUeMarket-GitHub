...
<tr><td class="active" onClick="location.href='message_area.php'"><center>收&nbsp;&nbsp;&nbsp;件&nbsp;&nbsp;&nbsp;&nbsp;夾</center></td></tr>
...
$sql = "select ms.username as sender, mr.username as reciever, mr.active, msg.* from member ms, member mr, message msg where mr.id = msg.to and ms.id = msg.from and mr.id='$user_id' and msg.receiver_status in (0,1) order by msg.id desc"; //在test資料表中選擇所有欄位
...
for($k = 0; $k < $number_of_row; $k ++) {
   ...
   if($id!=$row[id]){
    ...
    echo '<tr onClick="location.href=\'message_area.php?id='.$row[id].'\'"><form action="delete_message_re.php" method="post"><td>'.$row[sender].'</td><td>'.$row[subject].'</td><td width="100" align="right"><input type="submit" value="刪除" class="btn btn-danger"><input type="hidden" value="'.$row[id].'" name=id></td></form><tr>';
...
echo '<tr style="font-weight: bold; font-size:16px; text-decoration: underline;" onClick="location.href=\'message_area.php?id='.$row[id].'\'"><form action="delete_message_re.php" method="post"><td>'.$row[sender].'</td><td>'.$row[subject].'</td><td width="100" align="right"><input type="submit" value="刪除" class="btn btn-danger"><input type="hidden" value="'.$row[id].'" name=id></td></form><tr>';
    ...
    if($id==$row[id]){
        ...
        echo '<tr class="success" onClick="location.href=\'message_area.php\'"><form action="delete_message_re.php" method="post"><td>'.$row[sender].'</td><td>'.$row[subject].'</td><td width="100" align="right"><input type="submit" value="刪除" class="btn btn-danger"><input type="hidden" value="'.$row[id].'" name=id></td></form><tr>';
