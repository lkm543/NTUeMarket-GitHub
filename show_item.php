...
//echo $start.$per; 
  if($sort==null&$keyword==null){
    $sql2 = "select * from item_forsell where status=1 ".$order_by." limit ".$start.",".$per;
$result2 = mysqli_query($link,$sql2); // 執行SQL查詢
}
if($sort==null&$keyword!=null){
  $sql2 = "select * from item_forsell where (name like '%$keyword%' or detail like '%$keyword%') and status=1 ".$order_by." limit ".$start.",".$per;
$result2 = mysqli_query($link,$sql2); // 執行SQL查詢
}
if($sort!=null&$keyword==null){
  $sql2 = "select * from item_forsell where sort='$sort' and status=1 ".$order_by." limit ".$start.",".$per;
$result2 = mysqli_query($link,$sql2); // 執行SQL查詢
}
if($sort!=null&$keyword!=null){
  $sql2 = "select * from item_forsell where ((name like '%$keyword%' or detail like '%$keyword%') and sort='$sort') and status=1 ".$order_by." limit ".$start.",".$per;
$result2 = mysqli_query($link,$sql2); // 執行SQL查詢
}
...
<div class="row" style="padding-top:10px;">
  <div class="col-md-10 col-md-offset-1">
    ...
    <?php for($i=1;$i<=$pages;$i++) { 
      echo "<a href=\"show_item.php?keyword=".$keyword."&sort=".$sort."&sorting=".$sorting."&page=".$i."\"><span class=\"item_pagination\">".$i."</span></a>"; 
    }
    ?>
  </div>
</div>
